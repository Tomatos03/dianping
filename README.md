...
4. 整个流程在极端情况下可能出现redis数据和数据库数据不一致的问题. 怎么处理这个问题?
  答: 这个功能出现redis数据和数据库数据不一致的情况分两种:
   ...
   + redis挂了, MQ正常. 此时会丢失一些抢卷成功扣减的库存,  造成$redis优惠券库存 > 数据库优惠券库存$.  
     这种情况的处理方式:  进行人工干预,  先禁止程序对外服务, 等待MQ消息全部消费完毕, 同步数据库优惠券库存到redis优惠券库存.在开放程序服务

   + MQ挂了, redis正常. 此时抢卷功能仍然能够使用, 但$redis优惠券库存 < 数据库优惠券库存$  
     这种情况的处理方式:  开启一个定时任务, 定时检查redis优惠券库存是否和数据库库存一致, 如果不一致, 以redis中的数据为准做数据补偿
